...
import { ButtonCall, ButtonCallDocument } from './schemas/buttonCall.schema';
...
export class ButtonCallService {
  constructor(
    ...
    private buttonCallModel: Model<ButtonCallDocument>,
  ...
  {
    const tableEntry = Object.entries(tableCodes).find(([, value]) =>
        value.call_code === code || value.cancel_code === code,
      );
    if (!tableEntry) {
    ...
    if(code == call_code) {
      const existingButtonCall = await this.buttonCallModel.findOne({tableName: tableName, finishHour: ''});
      ...
      const closedButtonCall = await this.buttonCallModel.findOne({tableName: tableName, finishHour: ''});
      if(!closedButtonCall) {
  ...
  async close(user: User, closeButtonCallDto: CloseButtonCallDto, notifynodemcu = false) {
    ...
    const closedButtonCall = await this.buttonCallModel.findOne(
      { _id: closeButtonCallDto._id, finishHour: '' },
    );
    if (!closedButtonCall) {
    ...
    console.log("Button call cancelled successfully for ", closedButtonCall.tableName);
    this.buttonCallGateway.emitButtonCallChanged(closedButtonCall);

    if(notifynodemcu) {
      const tableEntry = tableCodes[closedButtonCall.tableName];
      this.udpService.sendUdpPacket(
        1,
        tableEntry.cancel_code
      );
    }
