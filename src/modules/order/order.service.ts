...
export class OrderService {
  ...
  async createOrder(user: User, createOrderDto: CreateOrderDto) {
    ...
    if (
      !createOrderDto.isOnlineSale &&
      !createOrderDto.table &&
      ![OrderStatus.WASTED].includes(createOrderDto.status as OrderStatus)
    ) {
      throw new HttpException(
  ...
  async simpleBulkOrderUpdate(
    ...
    try {
      ...
      if (updates.isShopifyCustomerPicked === true) {
        const uniqueShopifyOrderIds = [
          ...new Set(
            orders
              .filter((o) => o.shopifyCustomer && o.shopifyOrderId)
              .map((o) => o.shopifyOrderId as string),
          ),
        ];
          ...
          try {
            ...
            if (fulfillment?.id) {
              const firstOrder = orders.find(
                (o) => o.shopifyOrderId === shopifyOrderId,
              );
              if (firstOrder) {
                await this.orderModel.findByIdAndUpdate(firstOrder._id, {
                  shopifyFulfillmentId: fulfillment.id,
                });
              }
            }
            this.logger.log(
              `Shopify fulfillment created for Shopify order ${shopifyOrderId}`,
            );
          } catch (err) {
            this.logger.error(
              `Failed to create Shopify fulfillment for ${shopifyOrderId}:`,
              err,
            );
          }
        }
      } else if (updates.isShopifyCustomerPicked === false) {
        const ordersWithFulfillment = orders.filter(
          (o) => o.shopifyFulfillmentId,
        );
        for (const order of ordersWithFulfillment) {
          try {
            await this.shopifyService.cancelFulfillment(
              order.shopifyFulfillmentId,
            );
            await this.orderModel.findByIdAndUpdate(order._id, {
              $unset: { shopifyFulfillmentId: '' },
            ...
            this.logger.log(
              `Shopify fulfillment cancelled for order ${order._id}`,
            );
          } catch (err) {
            this.logger.error(
              `Failed to cancel Shopify fulfillment for order ${order._id}:`,
              err,
            );
          }
        }
      ...
      const updatedOrders = await this.orderModel.find({ _id: { $in: ids } });
      this.websocketGateway.emitOrderUpdated(updatedOrders);
      return updatedOrders;
    } catch (error) {
      this.logger.error('Error in simpleBulkOrderUpdate:', error);
      throw new HttpException(
  ...
  async cancelShopifyOrder(
    ...
    // Data collected inside transaction, used after commit for side effects
    let postCommitData: {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (isPartialCancel) {
          // Partial cancellation: split → cancelled part + remaining active part
          const remainingQuantity = order.quantity - quantity;
          const orderWithoutId = order.toObject();
          delete orderWithoutId._id;
          delete orderWithoutId.shopifyOrderLineItemId;
          ...
          if (isLastOrderInCollection) {
            // Single-order collection: create new active collection for remainder,
            ...
            // Multi-order collection: replace this order entry with remainder,
            // create a separate cancelled collection for audit
            await this.collectionModel.findByIdAndUpdate(
              collection._id,
              {
          ...
          // Full cancellation: cancel order and manage collection
          cancelledOrder = await this.orderModel.findByIdAndUpdate(
            order._id,
            {
              $set: {
          ...
          if (isLastOrderInCollection) {
            // Only order in collection: cancel the whole collection
            await this.collectionModel.findByIdAndUpdate(
              collection._id,
              {
                status: OrderCollectionStatus.CANCELLED,
            ...
            // Multi-order collection: remove this order, create cancelled audit collection
            await this.collectionModel.findByIdAndUpdate(
              collection._id,
              {
                $pull: { orders: { order: order._id } },
  ...
  async cancelHepsiburadaOrder(
    ...
    try {
      await session.withTransaction(async () => {
        const order = await this.orderModel
          .findOne({ hepsiburadaLineItemId }, null, { session })
          .populate('item');
        ...
        if (isPartialCancel) {
          ...
          const orderWithoutId = order.toObject();
          delete orderWithoutId._id;
          delete orderWithoutId.hepsiburadaLineItemId;
      ...
      this.logger.error('Error cancelling hepsiburada order:', error);
      if (error instanceof HttpException) {
        throw error;
      }
      throw new HttpException(
  ...
  async getRetailerItemSummary(id: number, query: RetailerOrdersQueryDto) {
    const retailer = await this.retailerModel.findById(id).lean();
    if (!retailer) {
      throw new HttpException('Retailer not found', HttpStatus.NOT_FOUND);
    }
