...
export class TableSchedule {
  ...
  async handleEveryMinuteWithoutTimezoneCheck() {
    this.logger.log(`🔄 [Cron] Checking every minute without timezone with 6 ${new Date().toISOString()}`);
    await this.tableService.notifyUnclosedTables();
  }
  
  @Cron('10 * 9 * * *')
  async handleEveryMinuteWithoutTimezoneCheck2() {
    this.logger.log(`🔄 [Cron] Checking every minute without timezone with 9 ${new Date().toISOString()}`);
    await this.tableService.notifyUnclosedTables();
  }
